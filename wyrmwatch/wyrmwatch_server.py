...
class WyrmWatchServer:
    ...
    async def process_task_queue(self):
        while not self.shutdown_event.is_set():
            try:
                payload, target_agent = await self.task_queue.get()
                await self.process_task(payload, target_agent)
        ...
        if topic.startswith("mcp/"):
            ...
            if self.loop and self.loop.is_running():
                asyncio.run_coroutine_threadsafe(
                    self.task_queue.put((payload, target_agent)),
                    self.loop
                )
            else:
                logger.error("Cannot process task: event loop not running")
